...
import (
	"go-rest-api/controller/request"
...
type ProductController struct {
	productService service.IProductSerice
}

func NewProductcontroller(productService service.IProductSerice) *ProductController {
	return &ProductController{
		productService: productService,
	}
}

func (productController *ProductController) RegisterRouters(e *echo.Echo) {
...
func (productController *ProductController) AddProduct(c echo.Context) error {
	...
	if err := productController.productService.Add(addProductRequest.ToModel()); err != nil {
...
func (productController *ProductController) UpdatePrice(c echo.Context) error {
	...
	if err != nil {
		return c.JSON(http.StatusBadRequest, response.ErrorResponse{
			ErrorDescription: "newPrice parameter is distrubted!",
		})
	}
...
func (productController *ProductController) DeleteProductById(c echo.Context) error {
	...
	if err := productController.productService.DeleteById(int64(productId)); err != nil {
		return c.JSON(http.StatusNotFound, response.ErrorResponse{
			ErrorDescription: err.Error(),
		})
	}
